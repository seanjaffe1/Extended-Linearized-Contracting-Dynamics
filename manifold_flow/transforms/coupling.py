...
class CouplingTransform(transforms.Transform):
    ...
    # Sean added to use Eflow batch jacobian
    def forward(self, inputs, context=None, full_jacobian=False):
        outputs, logabsdet =  self.forward_(inputs, context, full_jacobian=False)
        if full_jacobian:
            jacobian = various.batch_jacobian_eflow(lambda x: self.forward_(x, context, full_jacobian=False)[0], inputs, inputs.shape[-1])
            return outputs, jacobian
        return outputs, logabsdet

    def forward_(self, inputs, context=None, full_jacobian=False):
        ...
        if full_jacobian:
            ...
            print(jacobian_transform.shape)
            print(transform_split.shape)
            ...
            #     return outputs
            # outputs_b = jacob_free_output(inputs)
            # jacobian_b = various.batch_jacobian_eflow(jacob_free_output, inputs, inputs.shape[-1])
            # # compare
            # print(torch.allclose(outputs, outputs_b))
            # print(torch.allclose(jacobian, jacobian_b))
            # print(jacobian)
            # print(jacobian_b)
    ...
    def inverse(self, inputs, context=None, full_jacobian=False):
        outputs, logabsdet = self.inverse_(inputs, context, full_jacobian=False)
        if full_jacobian:
            jacobian = various.batch_jacobian_eflow(lambda x: self.inverse_(x, context, full_jacobian=False)[0], inputs, inputs.shape[-1])
            return outputs, jacobian
        return outputs, logabsdet
    
    def inverse_(self, inputs, context=None, full_jacobian=False):
        if inputs.dim() not in [2, 4]:
            raise ValueError("Inputs must be a 2D or a 4D tensor.")
        ...
        if full_jacobian:
            ...
            # Put together full Jacobian
            batchsize = inputs.size(0)
            jacobian = torch.zeros((batchsize,) + inputs.size()[1:] + inputs.size()[1:]).to(inputs.device)
            jacobian[:, self.identity_features, self.identity_features] = 1.0
            jacobian[:, self.transform_features, :] = jacobian_transform
